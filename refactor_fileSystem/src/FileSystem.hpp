...
private:
    ...
    uint64_t inodeOffset(uint64_t inodeId);                 // offset de un inode
    ...
    bool dirAdd(const std::string& name, uint64_t inodeId); // agrega entrada al directorio
    bool dirRemoveByIndex(int idx);                         // idx en directory
    bool loadDirectoryFromDisk();                           // carga directorio desde disco
    bool saveDirectoryToDisk();                             // guarda directorio en disco 
...
public:
    ...
    ~FileSystem();  // Destructor para cerrar disco y guardar cambios
