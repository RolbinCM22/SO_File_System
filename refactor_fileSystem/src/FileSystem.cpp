...
: disk(diskPath), superBlock{} 
{
    ...
    if (!disk.openDisk()) {
        // Si no existe o falla, lo creamos (trunc) y volvemos a abrir en rw
    ...
    auto looksFormatted = [&](const Layout::superBlock& s) -> bool {
        ...
        if (s.data_area_offset == 0)               return false; // offsets no inicializados
        // (podrías agregar más validaciones si querés)
        return true;
    };

    if (looksFormatted(onDisk)) {
        // Ya estaba formateado → usamos el superbloque del disco
        superBlock = onDisk;
        std::cout << "[FS] Disco previamente formateado detectado.\n";
        // Montamos estructuras en memoria
        if (!mount()) {
            ...
            // No reformateamos automáticamente para preservar datos
        }
    } else {
        // No formateado → formateamos y montamos
...
bool FileSystem::write(const std::string& name, const std::string& data) {
    ...
    for (int i = 0; i < 10 && remaining > 0; ++i) {
        ...
        // Si el chunk es menor al bloque, rellena con ceros el resto
    ...
    // (no se usa ctime/mtime/atime por ahora)
...
int FileSystem::allocateInode() {
    // Empezar desde 1, reservar inode 0 (raíz o inválido)
